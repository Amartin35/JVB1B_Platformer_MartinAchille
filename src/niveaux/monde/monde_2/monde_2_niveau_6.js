...
export default class MONDE_2_NIVEAU_6 extends Phaser.Scene{
	constructor() {
		super({key : "MONDE_2_NIVEAU_6"});
	}
	/////////////////////////////////////// CREATE ///////////////////////////////////////
	create(){
		// Map
		const map = this.add.tilemap("map_monde_2_niveau_6");
		...
		const paralaxeLayer = map.createLayer("Paralaxe", tileset).setDepth(PROPS_LAYER_DEPTH);
		const solideLayer = map.createLayer("Solide",tileset).setDepth(SOLIDE_LAYER_DEPTH);
        const arriereLayer = map.createLayer("Arriere",tileset).setDepth(ARRIERE_LAYER_DEPTH);
		...
		// Ajout class
		this.player = new Player(this, 20, 320, 'perso');
		...
		this.physics.add.collider(this.player, finLayer, () => {
			this.scene.start("MONDE_2_NIVEAU_7",{
			});
			console.log("switch");
		});
